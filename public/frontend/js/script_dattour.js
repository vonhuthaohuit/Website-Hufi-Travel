...
function addCustomerRow() {
    ...
    var index = document.querySelectorAll("#customerTable tbody tr").length + 1;

    newRow.querySelector('input[name="td_ticket[1][td_name]"]').name = `td_ticket[${index}][td_name]`;
    newRow.querySelector('input[name="td_ticket[1][td_birthday]"]').name = `td_ticket[${index}][td_birthday]`;
    newRow.querySelector('select[name="td_ticket[1][td_gender]"]').name = `td_ticket[${index}][td_gender]`;
    newRow.querySelector('select[name="td_ticket[1][td_loaikhach]"]').name = `td_ticket[${index}][td_loaikhach]`;
    newRow.querySelector('input[name="td_ticket[1][td_price]"]').name = `td_ticket[${index}][td_price]`;

    newRow.querySelector(".js-input-price").value = "2290000";
    newRow.querySelector(".td-price").innerText = "2,290,000";
...
function validateNameAndBirthday() {
    ...
    rows.forEach(function (row) {
        var name = row.querySelector('input[name^="td_ticket"][name$="[td_name]"]');
        var birthday = row.querySelector('input[name^="td_ticket"][name$="[td_birthday]"]');
...
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("tour_tep1").addEventListener("click", function (event) {
    ...
    function validateBookingForm() {
        ...
        hasError = checkRequiredField('ticket_fullname', "Trường này là bắt buộc!");
        hasError = checkRequiredField('ticket_address', "Trường này là bắt buộc!") || hasError;
    ...
    document.querySelectorAll('input[name="ticket_fullname"], input[name="ticket_address"], input[name="ticket_phone"], input[name="ticket_email"]').forEach((input) => {
