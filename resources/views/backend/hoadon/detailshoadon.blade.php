...
<div class="invoice-container">
    ...
    <p class="header"><strong>Ngày lập:</strong> {{ \Carbon\Carbon::parse($hoaDon->created_at)->format('d/m/Y') ?? '' }}</p>
    ...
    <table class="details">
        ...
        <tr>
            <td class="text-truncate"><strong>Địa chỉ:</strong> {{ '140 Lê Trọng Tấn, Phường Tây Thạnh, Quận Tân Phú, TP.HCM' ?? 'N/A' }}</td>
        ...
        <tbody>
            @foreach ($hoaDon->phieudattour->chitietphieudattour as $index => $chiTiet)
                <tr>
                    ...
                    <td>Vé</td>
                    <td>{{ count($hoaDon->phieudattour->chitietphieudattour) }}</td>
                    <td>{{ number_format($chiTiet->chitietsotiendat) }} VNĐ</td>
                </tr>
            @endforeach
        </tbody>
    </table>
